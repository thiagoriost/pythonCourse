...
async def process_order(order_id, items):
    ...
    # el metodo gather ejecuta procesos de forma concurrente

    if not all(inventory_results):#La función all() devuelve True si todos los elementos del iterable son verdaderos.
...
async def main():
    ...
    tasks = [process_order(order['order_id'], order['items']) for order in orders] # guarda un listados de funciones "process_order" a ejecutar
    await asyncio.gather(*tasks) # Ejecuta varias tareas asíncronas al mismo tiempo y esperar a que todas terminen.
    print("3333")
